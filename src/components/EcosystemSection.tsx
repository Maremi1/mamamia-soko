...
export const EcosystemSection: React.FC = () => {
  const getPortalIcon = (iconName: string, domain?: string) => {
    ...
    switch (iconName) {
      case 'Building2':
        return <Building2 className="w-6 h-6 text-amber-600 dark:text-amber-400" />;
      case 'ShoppingBag':
  ...
  return (
    <section id="ecosystem" className="py-20 bg-slate-100 dark:bg-slate-900 text-slate-900 dark:text-slate-100 relative transition-colors duration-300">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {ECOSYSTEM_PORTALS.map((portal) => (
            <a
              ...
              className="bg-white dark:bg-slate-800/85 rounded-2xl border border-slate-200 dark:border-slate-700/80 hover:border-amber-500/60 transition-all flex flex-col justify-between shadow-lg dark:shadow-2xl group hover:-translate-y-1 overflow-hidden backdrop-blur-md"
            >
              <div>
                {/* Portal Image Header */}
                {portal.imageUrl ? (
                  ...
                  <div className="p-6 pb-0 flex items-center justify-between">
                    <div className="p-3 bg-slate-100 dark:bg-slate-900 rounded-xl border border-slate-200 dark:border-slate-700 group-hover:border-amber-500/40 transition-colors">
                      {getPortalIcon(portal.icon, portal.domain)}
                    </div>
                    <span className="text-[10px] font-bold uppercase text-amber-700 dark:text-amber-300 bg-amber-500/10 px-2.5 py-1 rounded-full border border-amber-500/20">
                      {portal.badge}
                    </span>
                  </div>
                )}
                ...
                <div className="p-6 pt-4">
                  <div className="text-xs font-bold text-amber-700 dark:text-amber-400 tracking-wide uppercase">
                    {portal.focus}
                  </div>
                  <h3 className="text-xl font-bold text-slate-900 dark:text-white mt-1 group-hover:text-amber-600 dark:group-hover:text-amber-300 transition-colors">
                    {portal.domain}
                  </h3>
              ...
              <div className="px-6 pb-5 pt-3 border-t border-slate-200 dark:border-slate-700/80 flex items-center justify-between text-xs font-bold text-amber-600 dark:text-amber-400 group-hover:text-amber-700 dark:group-hover:text-amber-300 bg-slate-50 dark:bg-slate-900/40">
